...
export class AttachmentUpload extends Upload<AttachmentUploadOptions> implements FileUploadHandler {
  ...
  constructor(
    ...
    maxUploads: number,
    editorId: string,
    options: Partial<AttachmentUploadOptions>,
  ) {
    super(
      buttonContainerId,
      targetId,
    ...
    if (this._editorId) {
      ...
      EventHandler.add("com.woltlab.wcf.redactor2", "pasteFromClipboard_" + this._editorId, (data) =>
        this.editorUpload(data),
      );
      ...
      const metacodeAttachUuid = EventHandler.add(
        "com.woltlab.wcf.redactor2",
        "metacode_attach_" + this._editorId,
        (data) => {
      ...
      const syncUuid = EventHandler.add("com.woltlab.wcf.redactor2", "sync_" + this._tmpHash, (data) =>
        this.sync(data),
      );
    ...
    return resizer
      ...
      .then((result) => {
        ...
        if (window.devicePixelRatio >= 2) {
          ...
          if (
            realWidth - 10 < result.image.width &&
  ...
  protected onDelete(data: IDeleteData): void {
    ...
    const attachment: HTMLElement | null = this._target.querySelector(
      `.formAttachmentListItem[data-object-id="${objectId}"]`,
    );
    if (attachment !== null) {
      attachment.remove();
    }
    ...
    if (attachmentsFound && !this._target.classList.contains("sortableList")) {
      ...
      if (Environment.platform() === "desktop") {
        new window.WCF.Sortable.List(
          DomUtil.identify(this._target.parentNode as HTMLElement),
          "",
          0,
          {
            axis: false,
    ...
    this._target.querySelectorAll(":scope > li").forEach((element: HTMLElement) => {
      if (element.dataset.isImage) {
        images[~~(element.dataset.objectId || 0)] = {
          thumbnailUrl:
            (element.querySelector(".jsButtonAttachmentInsertThumbnail") as HTMLSpanElement).dataset.url || null,
  ...
  protected registerEditorButtons(attachment: HTMLElement): void {
    if (this._editorId) {
      attachment
        .querySelectorAll(
          ".jsButtonAttachmentInsertThumbnail, .jsButtonAttachmentInsertFull, .jsButtonAttachmentInsertPlain",
        )
        .forEach((button) => {
